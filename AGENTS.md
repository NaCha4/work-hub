...
- **업로드한 HTML 은 gzip 으로 눌러 담는다**(`htmlz`, base64). Firestore 문서 상한
  1 MiB 는 하드 리밋이라 늘릴 수 없고, HTML 은 잘 눌려서 실질 한도가 몇 배가 된다.
  브라우저 기본 기능(`CompressionStream`)이라 의존성은 그대로다. 무손실이므로
  내려받기와 발표 화면 모두 원본과 바이트가 같다.
  다만 **이미지를 박아 넣은 자료는 거의 줄지 않는다**(실측 1.0배). 이미 눌린
  바이트라서다. 그런 자료가 한도에 걸리면 압축을 손볼 게 아니라 저장 위치를 옮겨야 한다.
- 예전에 압축 없이 올린 자료(`html`)와 마크다운 자료(`content`)도 계속 열린다.
  `resolvePrepHtml()` 한 곳에서 세 경우를 모두 처리하므로 호출부는 구분하지 않는다.
