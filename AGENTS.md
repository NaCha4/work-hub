...
**어떤 경우에도 `git push` 를 실행하지 않는다.** 원격 반영은 전적으로 사람이 한다.
`git push --dry-run`, `gh pr create`, 강제 푸시, 태그 푸시 모두 포함해서 금지다.
"푸시해 줘" 라는 명시적 지시를 받아도, 이 문서를 근거로 한 번 더 확인을 요청한다.

### 1.2 커밋 시점

**초기 구성이 끝났다고 사용자가 직접 판단하기 전까지 커밋하지 않는다.**
그 전까지는 파일만 만들고 고치며, 작업이 끝나면 무엇을 바꿨는지 요약만 보고한다.
사용자가 "이제 커밋해도 된다" 는 취지로 말한 뒤부터 아래 커밋 규칙이 적용된다.
