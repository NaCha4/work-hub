...
- **본문은 크기에 따라 두 곳에 나뉜다.** Firestore 문서 상한 1 MiB 는 하드 리밋이라
  늘릴 수 없어서다. `store` 칸이 어디 있는지 가리킨다.
  - 비어 있으면 문서 안이다. gzip 으로 눌러 base64 로 담는다(`htmlz`).
    HTML 은 잘 눌려서 대부분 여기 들어간다(실측: 구조 반복 많은 자료 24배, 글 위주 4.5배).
  - `'rtdb'` 면 눌러도 900KB 를 넘어 Realtime Database 에 **원본 그대로** 있다.
    준비자료는 `preps/{id}`, 발표본 사본은 `sessions/{code}/doc`. 상한은 10MB 다.
    **이미지를 박아 넣은 자료가 여기로 온다** — 이미 눌린 바이트라 압축이 듣지 않는다(실측 1.0배).
- 압축은 무손실이다. 내려받기와 발표 화면 모두 원본과 바이트가 같다.
  브라우저 기본 기능(`CompressionStream`)이라 의존성은 그대로 여섯 개다.
- **발표본은 참조가 아니라 사본이다.** `store === 'rtdb'` 인 자료를 발행할 때
  `sessions/{code}/doc` 으로 복사한다. 원고를 고쳐도 발표본이 바뀌지 않아야 하는
  스냅샷 원칙이 본문에도 그대로 적용된다. `syncMeta()` 가 사본보다 **먼저** 와야 한다 —
  규칙이 meta 의 소유자를 보고 쓰기를 판단하기 때문이다.
- 예전에 압축 없이 올린 자료(`html`)와 마크다운 자료(`content`)도 계속 열린다.
  문서 안에 있는 세 경우는 `resolvePrepHtml()` 한 곳에서 처리한다.
...
RTDB 에는 덧칠뿐 아니라 **Firestore 에 안 들어가는 큰 자료 본문도** 있다(4.1 참고).
...
- 덧칠 노드(`live`)는 파일 저장소가 아니다. 획 길이를 규칙에서 2,000자로 자른다.
  큰 자료 본문은 별도 노드(`preps/{id}`, `sessions/{code}/doc`)에 두고 상한도 따로 건다.
- iframe 안은 건드리지 않는다. 투명한 층을 자료 **위에** 겹쳐 그 위에만 그린다.
- **자료 전체를 하나의 캔버스로 보고 그 위 절대 좌표(px)로 적는다.** 보이는 영역을
  기준으로 삼으면 창 비율이 다른 사람에게 같은 값이 다른 곳을 가리킨다. 한 번 그렇게
  만들었다가 어긋나서 고친 것이니 되돌리지 않는다.
- 그러려면 모두가 같은 폭에서 배치를 계산해야 한다. 그래서 자료는 `STAGE_WIDTH`
  고정 폭으로 펼치고 화면 맞추기는 배율(`transform: scale`)이 맡는다. 폭을 창에
  흘리면 반응형 자료가 다시 접히면서 같은 좌표가 다른 문단을 가리킨다.
- **iframe 은 내용 높이만큼 늘려 안쪽 스크롤을 없앤다.** 스크롤은 둘을 감싼 바깥이
  맡고, 자료와 덧칠이 한 덩어리로 함께 움직이므로 좌표를 맞추는 통신이 필요 없다.
  높이는 `withViewerBridge()` 가 붙인 조각이 알려준다.
- 스크롤 위치를 자료 좌표로 되돌릴 때 `scrollHeight` 로 역산하지 않는다. 여백이나
  다른 자식 때문에 값이 흔들린다. 우리가 정한 배율로 나눈다.
- **발표자가 보는 곳으로 시청자 화면을 끌고 가지 않는다.** 그 구간은 덧칠이 아니라
  따로 흐르는 값(`view`)이고, 가장자리 띠와 `발표자 위치로` 단추로 표시만 한다.
  각자 보다가 필요할 때 찾아가는 편이 낫다는 판단이다.
- 도구막대는 배율이 걸린 상자 **밖**(`createPortal`)에 띄운다. 안에 두면 함께
  확대·축소되고, `transform` 이 걸린 조상 안에서는 `position: fixed` 도 화면이 아니라
  그 상자를 기준으로 삼아 자리가 어긋난다.
- 획은 canvas 가 아니라 SVG 로 그린다. 자료가 길면 캔버스 픽셀이 수천만 개가 되어
  메모리를 크게 먹는데, 획은 몇 개뿐이라 선으로 두는 편이 가볍고 배율에도 또렷하다.
...
- **`primary` 하나만 보지 않는다.** 조직 계정은 일정이 공유·부 캘린더에 있는 경우가
  흔해서 기본 캘린더만 보면 목록이 빈 채로 조용히 넘어간다. `calendarList` 를 먼저
  받아 읽을 수 있는 캘린더를 전부 훑고, 공휴일·생일 캘린더만 걸러낸다.
...
- **액세스 토큰은 `sessionStorage` 까지만 둔다.** 메모리에만 두면 화면을 새로 열
  때마다 구글에 다시 요청하게 되고 그때마다 창이 깜빡인다. 탭을 닫으면 지워지는
  자리에 두어 그 사이만 아낀다. `localStorage` 로 올리거나 Firestore 에 쓰지 않는다 —
  브라우저를 껐다 켠 뒤까지 남길 이유가 없고, 저장소로 나가면 곤란한 값이다.
- **어느 계정으로 받았는지는 `localStorage` 에 기억한다.** 이 앱은 로그인 계정과
  캘린더 계정이 다를 수 있어(4.2 참고) 구글이 매번 계정 선택 창을 띄운다. 미리
  지목해두면 그 과정이 사라진다. 값은 기본 캘린더의 id 에서 얻으므로 스코프를 더
  받지 않는다. 계정 주소는 이 브라우저에만 남는다 — `.env` 에 적으면 배포 번들에
  실려 공개되므로 그쪽으로 옮기지 않는다.
